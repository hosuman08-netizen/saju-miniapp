  ...
  try{
    ...
    function channel(){
      try{
        ...
        // 🩹 2026-08-13: startapp만 읽어 웹 공유(?ref/utm_source) 유입이 전부 direct로 뭉개지던 것 보완
        if(ch==='direct'){
          var q=location.search||'';
          var mu=q.match(/[?&](?:utm_source|source)=([^&]+)/); var mr=q.match(/[?&]ref=([^&]+)/);
          var cand=mu?decodeURIComponent(mu[1]):(mr?decodeURIComponent(mr[1]):'');
          if(cand){ var mm=/^ref_\d+_?(\w*)$/.exec(cand); if(mm) cand=mm[1]||'ref';
            ch=cand.toLowerCase().replace(/[^a-z0-9]/g,'').slice(0,16)||'direct'; }
        }
    ...
    window.legionTrack = function(type, extra){
      try{
        ...
        // 2026-07-29 C4: multi worker reads b.anon (anonId-only → DAU collapse as "anon") — mirror p21
