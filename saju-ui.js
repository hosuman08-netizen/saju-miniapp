...
(function () {
  ...
  /* ================= 도시 셀렉트 초기화 ================= */
  var LON_PRESETS = ['서울', '부산', '제주', '도쿄', '뉴욕'];
  function lonRefMin(lon) {
    return Math.round(Number(lon) * 4 - 540);
  }
  function fmtLon(lon) {
    lon = Number(lon);
    if (lon < 0) return '서경 ' + Math.abs(lon).toFixed(3) + '°';
    return '동경 ' + lon.toFixed(3) + '°';
  }
  function paintLonHint() {
    var sel = $('city'); if (!sel) return;
    var lon = parseFloat(sel.value);
    var opt = $('optLon');
    var on = !opt || opt.checked;
    var city = sel.options[sel.selectedIndex] ? sel.options[sel.selectedIndex].textContent : '';
    var m = lonRefMin(lon);
    var hint = $('lonHint');
    if (hint) {
      hint.textContent = city + ' · ' + fmtLon(lon) + ' · 표준자오선 135° 대비 ' +
        (m >= 0 ? '+' : '') + m + '분' + (on ? ' (보정 ON)' : ' (보정 OFF · 표준시 그대로)') +
        ' · 시계=현지시각 · 예언 아님';
    }
    var box = $('lonPreset');
    if (box) {
      Array.prototype.forEach.call(box.querySelectorAll('[data-loncity]'), function (b) {
        b.classList.toggle('on', b.getAttribute('data-loncity') === city);
      });
    }
  }
  function initLonPreset() {
    var box = $('lonPreset');
    if (!box || box.getAttribute('data-ready')) return;
    box.setAttribute('data-ready', '1');
    LON_PRESETS.forEach(function (name) {
      var city = null;
      SP.CITIES.forEach(function (c) { if (c.name === name) city = c; });
      if (!city) return;
      var b = document.createElement('button');
      b.type = 'button';
      b.className = 'lon-chip';
      b.setAttribute('data-loncity', name);
      b.textContent = name + ' ' + (city.lon < 0 ? 'W' : 'E') + Math.abs(city.lon).toFixed(0);
      b.onclick = function () {
        var sel = $('city'); if (!sel) return;
        var i, found = -1;
        for (i = 0; i < sel.options.length; i++) {
          if (sel.options[i].textContent === name) { found = i; break; }
        }
        if (found >= 0) sel.selectedIndex = found;
        paintLonHint();
        if (window.legionTrack) window.legionTrack('lon_preset', { city: name });
      };
      box.appendChild(b);
    });
    var sel = $('city');
    if (sel) sel.onchange = paintLonHint;
    var opt = $('optLon');
    if (opt) {
      var prev = opt.onchange;
      opt.onchange = function () { if (prev) prev(); paintLonHint(); };
    }
    paintLonHint();
  }
  /* 한국 서머타임만. 지역 DST 발명 없음. 예언 아님. */
  function dstYearHit(y) {
    var months = [5, 6, 7, 8, 9];
    var i;
    for (i = 0; i < months.length; i++) {
      if (SP.dstMin && SP.dstMin(y, months[i], 15, 12)) return true;
    }
    return false;
  }
  /* 표기시각 기준. 엔진은 tst.h>=23 + 조자시만 일주 이월. 예언 발명 없음. */
  function jaWindow(hour, unknown) {
    if (unknown) return 'none';
    hour = Number(hour);
    if (hour === 23) return 'split';
    if (hour === 0) return 'late';
    return 'out';
  }
  function paintJaHint() {
    var hint = $('jaHint');
    if (!hint) return;
    var tu = $('timeUnknown');
    var unknown = !!(tu && tu.checked);
    var timeEl = $('time');
    var hm = (timeEl && timeEl.value ? timeEl.value : '12:00').split(':').map(Number);
    var school = $('jaSchool') ? $('jaSchool').value : '조자시';
    var w = jaWindow(hm[0] || 0, unknown);
    if (w === 'none') {
      hint.textContent = '시각 모름 · 시주 제외 · 자시 학파 영향 없음 · 예언 아님';
    } else if (w === 'split') {
      hint.textContent = '표기 23시 = 학파 분기 · 조자시=다음날 일주 · 야자시=당일 일주 · 지금 ' + school + ' · 예언 아님';
    } else if (w === 'late') {
      hint.textContent = '표기 0시대 = 자시 후반 · 엔진은 23시만 분기 · 일주 학파차 없음 · 지금 ' + school + '';
    } else {
      hint.textContent = '자시 구간 아님 (23:00–00:59만 학파 분기) · 지금 ' + school + ' · 예언 아님';
    }
    hint.classList.toggle('warn', w === 'split');
    var box = $('jaChip');
    if (box) {
      Array.prototype.forEach.call(box.querySelectorAll('[data-ja]'), function (b) {
        b.classList.toggle('on', b.getAttribute('data-ja') === school);
      });
    }
  }
  function initJaChip() {
    ...
    if (!box || box.getAttribute('data-ready')) return;
    box.setAttribute('data-ready', '1');
    [['조자시', '조자시 23시→다음날'], ['야자시', '야자시 자정→다음날']].forEach(function (pair) {
      var b = document.createElement('button');
      b.type = 'button';
      b.className = 'lon-chip';
      b.setAttribute('data-ja', pair[0]);
      b.textContent = pair[1];
      b.onclick = function () {
        var sel = $('jaSchool');
        if (sel) sel.value = pair[0];
        paintJaHint();
        if (window.legionTrack) window.legionTrack('ja_chip', { ja: pair[0] });
      };
      box.appendChild(b);
    });
    var sel = $('jaSchool');
    if (sel) {
      var prev = sel.onchange;
      sel.onchange = function () { if (prev) prev(); paintJaHint(); };
    }
    paintJaHint();
  }
  /* 결과탭 학파 대비. 엔진 computePro 두 번. 토정/꿈/예언 발명 없음. */
  function jaCompareAt(y, m, d, hour, minute, opt) {
    opt = opt || {};
    if (opt.timeKnown === false) {
      return { same: true, split: false, joH: '', yaH: '', applied: opt.ja || '조자시',
        line: '시각 모름 · 학파 대비 없음 · 예언 아님' };
    }
    var jo = SP.computePro(y, m, d, hour, minute, {
      lon: opt.lon, ja: '조자시', timeKnown: true,
      applyLongitude: opt.applyLongitude, applyEot: opt.applyEot
    });
    var ya = SP.computePro(y, m, d, hour, minute, {
      lon: opt.lon, ja: '야자시', timeKnown: true,
      applyLongitude: opt.applyLongitude, applyEot: opt.applyEot
    });
    var applied = opt.ja || '조자시';
    var joH = jo.day && jo.day.hanja;
    var yaH = ya.day && ya.day.hanja;
    var same = joH === yaH;
    var split = !!(jo.meta && jo.meta.dayAdvanced);
    var line = same
      ? ('조자시·야자시 일주 동일 ' + joH + ' · 학파차 없음 · 적용 ' + applied + '')
      : ('조자시 ' + joH + ' vs 야자시 ' + yaH + ' · 적용 ' + applied
        + (applied === '조자시' && split ? ' (이월)' : '') + ' · 예언 아님');
    return { same: same, split: split, joH: joH, yaH: yaH, applied: applied, line: line };
  }
  function jaCompareLine(chart) {
    var inp = STATE.input || {};
    if (!inp.birth) return '학파 대비 대기';
    var ymd = inp.birth.split('-').map(Number);
    var hm = (inp.time || '12:00').split(':').map(Number);
    var r = jaCompareAt(ymd[0], ymd[1], ymd[2], hm[0] || 0, hm[1] || 0, {
      lon: inp.lon, ja: (chart && chart.meta && chart.meta.jaSchool) || inp.ja,
      timeKnown: !inp.timeUnknown,
      applyLongitude: inp.applyLongitude, applyEot: inp.applyEot
    });
    return r.line;
  }
  /* 결과탭 학파칩 → 엔진 재계산만. 토정/꿈/예언 발명 없음. */
  function paintJaResultChip() {
    var box = $('jaResultChip');
    if (!box) return;
    var school = (STATE.chart && STATE.chart.meta && STATE.chart.meta.jaSchool)
      || ($('jaSchool') && $('jaSchool').value) || '조자시';
    var unk = !!(STATE.input && STATE.input.timeUnknown);
    box.innerHTML = '';
    [['조자시', '결과 조자시'], ['야자시', '결과 야자시']].forEach(function (pair) {
      var b = document.createElement('button');
      b.type = 'button';
      b.className = 'lon-chip' + (pair[0] === school ? ' on' : '');
      b.setAttribute('data-ja-result', pair[0]);
      b.textContent = pair[1];
      if (unk) {
        b.disabled = true;
        b.title = '시각 모름 · 학파 전환 없음';
      } else {
        b.onclick = function () { applyJaSchool(pair[0]); };
      }
      box.appendChild(b);
    });
  }
  function applyJaSchool(school) {
    if (school !== '조자시' && school !== '야자시') return false;
    var sel = $('jaSchool');
    if (sel) sel.value = school;
    paintJaHint();
    var chartEl = $('chart');
    if (STATE.chart && chartEl && !chartEl.classList.contains('hidden')) {
      generate({ quiet: true });
    } else {
      paintJaResultChip();
    }
    if (window.legionTrack) window.legionTrack('ja_result_chip', { ja: school });
    return true;
  }
  /* 적용 일주 한자 칩. 엔진 day.hanja만. 토정/꿈/예언 발명 없음. */
  function appliedDayHanja(chart) {
    if (!chart || !chart.day) return '';
    return String(chart.day.hanja || '');
  }
  function jaDayChipLine(chart) {
    var h = appliedDayHanja(chart);
    if (!h) return '적용 일주 대기 · 예언 아님';
    var school = (chart.meta && chart.meta.jaSchool) || '조자시';
    return '적용 일주 ' + h + ' · ' + school + ' · 엔진 · 예언 아님';
  }
  function paintJaDayChip() {
    var el = $('jaDayHanja');
    if (!el) return;
    el.textContent = jaDayChipLine(STATE.chart);
    bindJaDayCopy();
  }
  /* 적용 일주 한자 복사. 엔진 day.hanja만. 토정/꿈/예언 발명 없음. */
  function copyAppliedDayHanja() {
    var h = appliedDayHanja(STATE.chart);
    if (!h) { toast('명식을 먼저 뽑으세요'); paintJaDayCopyOk(''); return false; }
    function done() {
      paintJaDayCopyOk(h);
      toast('적용 일주 한자 복사됨 · 엔진 · 예언 아님');
      if (window.legionTrack) window.legionTrack('ja_day_copy', { h: h });
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(h).then(done).catch(function () {
        window.prompt('아래를 복사하세요', h);
        done();
      });
    } else {
      window.prompt('아래를 복사하세요', h);
  ...
  /* 복사 확인 1줄. 엔진 day.hanja만. 가짜명리 0. WAVE213: 3초 자동숨김. 탭숨김 없음. */
  function jaDayCopyOkLine(h) {
    if (!h) return '';
    return '복사됨 · ' + String(h) + ' · 엔진 · 예언 아님';
  }
  var JA_DAY_COPY_OK_HIDE_MS = 3000;
  var jaDayCopyOkHideTimer = null;
  function jaDayCopyOkHideMs() { return JA_DAY_COPY_OK_HIDE_MS; }
  function hideJaDayCopyOk() {
    jaDayCopyOkHideTimer = null;
    var el = $('jaDayCopyOk');
    if (!el) return false;
    el.textContent = '';
  ...
  function armJaDayCopyOkHide() {
    if (jaDayCopyOkHideTimer) { clearTimeout(jaDayCopyOkHideTimer); jaDayCopyOkHideTimer = null; }
    jaDayCopyOkHideTimer = setTimeout(hideJaDayCopyOk, JA_DAY_COPY_OK_HIDE_MS);
  }
  function paintJaDayCopyOk(h) {
    var el = $('jaDayCopyOk');
    if (!el) return;
    var line = jaDayCopyOkLine(h);
    el.textContent = line;
    if (line) {
  ...
  function bindJaDayCopy() {
    var btn = $('jaDayCopy');
    if (!btn) return;
    btn.onclick = copyAppliedDayHanja;
    btn.disabled = !appliedDayHanja(STATE.chart);
    paintJaDayCopyOk('');
  }
  function paintDstHint() {
    var hint = $('dstHint');
    if (!hint || !SP.dstMin) return;
    var birth = $('birth');
    if (!birth || !birth.value) {
      hint.textContent = '한국 서머타임: 생년월일 입력 · 예언 아님';
      return;
    }
    var ymd = birth.value.split('-').map(Number);
    var timeEl = $('time');
    var tu = $('timeUnknown');
    var hm = (timeEl && timeEl.value ? timeEl.value : '12:00').split(':').map(Number);
    if (tu && tu.checked) hm = [12, 0];
    var applied = SP.dstMin(ymd[0], ymd[1], ymd[2], hm[0] || 0);
    if (applied) {
      hint.textContent = '한국 서머타임 적용 구간 · +' + applied + '분 · 시주가 한 칸 밀릴 수 있음 · 예언 아님';
    } else if (dstYearHit(ymd[0])) {
      hint.textContent = ymd[0] + '년은 서머타임 시행년 · 이 날짜·시각은 구간 밖 · 예언 아님';
    } else {
      hint.textContent = '한국 서머타임 해당없음 (시행: 1948–51 · 1955–60 · 1987–88) · 지역 DST 발명 없음';
    }
  }
  function initCities() {
    var sel = $('city'); if (!sel || sel.options.length) { initLonPreset(); return; }
  ...
  function generate(opts) {
    ...
    STATE.gender = gender;
    STATE.input = {
      birth: birth, time: time, timeUnknown: timeUnknown, lon: lon, ja: ja,
      applyLongitude: $('optLon').checked, applyEot: $('optEot').checked
    };

    // 최근 프로필 루프 (원탭 재조회). quiet=결과칩 재계산 → 조회수/스트릭 안 올림.
    if (!opts.quiet) try {
    ...
    renderHero(); renderGijil(); renderMyeongsik(); renderElements();
    ...
    } else if (window.legionTrack) {
      window.legionTrack('ja_recalc', { ja: ja });
    }
  }
  ...
  function renderHero() {
    ...
    $('hero').innerHTML =
      ...
      (c.meta && c.meta.dstApplied ? '<span class="chip" id="dstChip">서머타임 +' + (c.meta.dstMin || 60) + '분</span>' : '') +
      '</div>' +
      '<p class="hero-gyeok">' + esc(SP.GYEOK_MEAN[g.name] || '') + '</p>' +
      (g.basis ? '<p class="hero-basis" id="gyeokBasis">판정 근거: ' + esc(g.basis) + '</p>' : '') +
      '<p class="hero-basis" id="jaCompare">' + esc(jaCompareLine(c)) + '</p>' +
      '<div id="jaResultChip" class="lon-preset" role="group" aria-label="결과 학파 전환"></div>' +
      '<p class="hero-basis" id="jaDayHanja">' + esc(jaDayChipLine(c)) + '</p>' +
      '<div id="jaDayCopyWrap" class="lon-preset"><button type="button" class="lon-chip" id="jaDayCopy">적용 일주 한자 복사</button></div>' +
      '<p class="hero-basis" id="jaDayCopyOk" hidden></p>' +
      '</div>';
    paintJaResultChip();
    paintJaDayChip();
  }

  /* 기질·시기·실행 — 명식/대운/용신에서만 파생. 발명 금지. */
  var EL_TOUCH = {
    목: '성장·기획·교육·창작',
    화: '표현·예술·홍보',
    토: '중재·관리·신뢰',
  ...
  function renderGijil() {
    var box = $('gijilCard');
    if (!box || !STATE.chart || !STATE.anal) return;
    var c = STATE.chart, a = STATE.anal;
    var dm = c.dayMaster;
    var g = SP.gyeokguk(c);
    var nowY = new Date().getFullYear();
    var cur = SP.currentDaeun(STATE.daeun, c, nowY);
    var seasonTxt = cur.cur
      ? '지금 대운 ' + cur.cur.stem + cur.cur.branch + ' · 천간 ' + cur.cur.stemGod
        + ' — ' + (SP.TEN_GOD_MEAN[cur.cur.stemGod] || '')
      : '첫 대운(' + STATE.daeun.startAgeInt + '세) 전 · 월주 기운을 그대로 씀';
    var y0 = a.yongsin && a.yongsin[0];
    var doTxt = y0
      ? '용신 ' + a.yongsin.join('·') + ' — ' + (EL_TOUCH[y0] || '') + ' 쪽을 의식적으로'
      : '용신 계산 후 방향을 읽습니다';
    box.innerHTML =
      '<div class="card gijil-card">' +
      '<h3 class="card-h">기질 · 시기 · 실행</h3>' +
      '<div class="gijil-grid">' +
      '<div class="gijil-cell"><b>기질</b><p>' + esc(ARCHE[dm] || '') +
        (g && g.name ? ' · ' + esc(g.name) : '') + '</p></div>' +
      '<div class="gijil-cell"><b>시기</b><p>' + esc(seasonTxt) + '</p></div>' +
      '<div class="gijil-cell"><b>실행</b><p>' + esc(doTxt) + '</p></div>' +
      '</div>' +
      '<p class="note">일간·격국·대운 십신에서만 읽음. 예언·적중 아님 · 엔터테인먼트</p>' +
      '</div>';
  }
  ...
  function renderBasis() {
    ...
    $('basisBody').innerHTML =
      ...
      '</div>' +
      '<div class="card" id="studyPromptCard">' +
      '<h3 class="card-h">정밀 명식 · 학습 프롬프트</h3>' +
      '<p class="note">천을귀인형 export. 엔진이 계산한 간지·설정만. 토정·꿈·예언 문장 없음.</p>' +
      '<pre id="studyPromptPre" class="study-pre"></pre>' +
      '<button type="button" class="secondary wide" id="copyStudyPrompt">학습 프롬프트 복사</button>' +
      '<p class="note">외부 AI에 붙여넣기용 · 엔터테인먼트 · 적중·조언 아님</p>' +
      '</div>' +
      '<p class="disclaimer">명식 계산은 결정적 알고리즘입니다. 해석 문장은 엔터테인먼트 목적이며 보장된 예측이 아닙니다. ' +
      '입력한 생년월일시는 이 기기 안에서만 계산되며 서버로 전송되지 않습니다.</p>';
    var pre = $('studyPromptPre');
    if (pre) pre.textContent = buildStudyPrompt();
    var btn = $('copyStudyPrompt');
    if (btn) btn.onclick = copyStudyPrompt;
    void c;
  }

  function pillarLine(p, label) {
    if (!p) return label + ': (시각 모름 — 시주 없음)';
    return label + ': ' + p.stem + p.branch + ' (' + p.hanja + ') · ' + p.el + '/' + p.bel + ' · ' + p.yy;
  }

  function buildStudyPrompt() {
    var c = STATE.chart, a = STATE.anal, m = c && c.meta;
    if (!c || !m) return '';
    var g = SP.gyeokguk(c);
    var yong = (a && a.yongsin && a.yongsin.length) ? a.yongsin.join('·') : '(없음)';
    var gi = (a && a.gisin && a.gisin.length) ? a.gisin.join('·') : '(없음)';
    var lines = [
      '[만세력 계산 결과 — 이 블록만 사용. 없는 글자·토정·꿈해몽·적중 예언을 만들지 마라.]',
      pillarLine(c.year, '년주'),
      pillarLine(c.month, '월주'),
      pillarLine(c.day, '일주'),
      pillarLine(c.hour, '시주'),
      '일간: ' + c.dayMaster + (c.day ? ' (' + c.day.hanja + ')' : ''),
      '격국: ' + g.name + ' — 근거: ' + g.basis,
      '용신: ' + yong + ' · 기신: ' + gi,
      '설정: 동경 ' + m.lon + '° · 경도보정 ' + (m.lonCorrMin !== 0 ? 'ON (' + m.lonCorrMin + '분)' : 'OFF') +
        ' · 균시차 ' + (m.eotMin !== 0 ? 'ON (' + m.eotMin + '분)' : 'OFF') +
        ' · ' + m.jaSchool + '설' + (m.dayAdvanced ? ' (일주 다음날 이월)' : '') +
        ' · 서머타임 ' + (m.dstApplied ? '적용 +60분' : '해당없음'),
      '진태양시: ' + m.trueSolar + ' (시계 ' + m.clockTime + ')',
      '연주 기준 입춘: ' + m.ipchun,
      '월주 절기: ' + m.monthTerm,
      '다음 절기: ' + (m.nextTerm || '–'),
      '표준시: ' + m.eraNote,
      '',
      '지시: 위 간지와 설정만으로 학습 설명을 하라. 원문에 없는 토정·꿈·관상을 발명하지 마라.',
      '단정·적중·미래보장 금지. 엔터테인먼트 · 의료·법률·투자 조언 아님 허구 프레임.'
    ];
    return lines.join('\n');
  }

  function copyStudyPrompt() {
    var text = buildStudyPrompt();
    if (!text) { toast('명식을 먼저 뽑으세요'); return; }
    function done() {
      toast('학습 프롬프트 복사됨');
      if (window.legionTrack) window.legionTrack('export', { kind: 'study_prompt' });
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(done).catch(function () {
        window.prompt('아래를 복사하세요', text);
        done();
      });
    } else {
      window.prompt('아래를 복사하세요', text);
  ...
  function invite() {
    ...
    // 🩹 2026-08-13 귀속 복구: 초대 링크에 채널/추천인이 없어 함대 최고 K 메커니즘이 계측상 투명인간이었음.
    var _kid = ''; try { _kid = (localStorage.getItem('legion_anon') || '').slice(0, 12); } catch (e) {}
    var url = location.origin + location.pathname + '?saju_invite=' + encodeURIComponent(enc)
      + '&startapp=' + encodeURIComponent('c-invite' + (_kid ? '-r-' + _kid : ''));
  ...
  function boot() {
    initCities(); initTabs();
    var tu = $('timeUnknown');
    if (tu) tu.onchange = function () { $('time').disabled = tu.checked; paintDstHint(); paintJaHint(); };
    var birth = $('birth');
    var timeEl = $('time');
    if (birth) birth.onchange = function () { paintDstHint(); paintJaHint(); };
    if (timeEl) timeEl.onchange = function () { paintDstHint(); paintJaHint(); };
    initJaChip();
    paintDstHint();
    paintJaHint();
  }
  ...
  var _gen = generate;
  generate = function (opts) { _gen(opts); setTimeout(renderRecent, 50); setTimeout(acceptInvite, 60); };

  window.SajuUI = { generate: generate, matchRun: matchRun, shareCard: shareCard, editInput: editInput, toast: toast, state: STATE, renderRecent: renderRecent, invite: invite, copyStudyPrompt: copyStudyPrompt, buildStudyPrompt: buildStudyPrompt, lonRefMin: lonRefMin, paintLonHint: paintLonHint, paintDstHint: paintDstHint, dstYearHit: dstYearHit, jaWindow: jaWindow, paintJaHint: paintJaHint, initJaChip: initJaChip, jaCompareAt: jaCompareAt, jaCompareLine: jaCompareLine, applyJaSchool: applyJaSchool, paintJaResultChip: paintJaResultChip, appliedDayHanja: appliedDayHanja, jaDayChipLine: jaDayChipLine, paintJaDayChip: paintJaDayChip, copyAppliedDayHanja: copyAppliedDayHanja, bindJaDayCopy: bindJaDayCopy, jaDayCopyOkLine: jaDayCopyOkLine, paintJaDayCopyOk: paintJaDayCopyOk, hideJaDayCopyOk: hideJaDayCopyOk, armJaDayCopyOkHide: armJaDayCopyOkHide, jaDayCopyOkHideMs: jaDayCopyOkHideMs };
